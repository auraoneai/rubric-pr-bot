import type { Finding, ReviewResult, RubricDiff } from "./rubric_review.js";

export function buildComment(diff: Pick<ReviewResult, "files" | "diffs">, findings: Finding[]): string {
...
function collectChangedCriteria(diffs: Array<{ file: string } & RubricDiff>): string[] {
  const ids = new Set<string>();
  for (const diff of diffs) {
    for (const id of diff.added) ids.add(id);
    for (const id of diff.removed) ids.add(id);
    for (const item of diff.changed) {
      if (item.criterion_id) ids.add(item.criterion_id);
    }
  }
  return [...ids].sort();
}
